...
@Component({
  selector: 'app-view-purchase-invoices',
  standalone: true,
  imports: [CommonModule, FilterPipe, HighlightPipe, FormsModule, TranslateModule],
  templateUrl: './view-purchase-invoices.component.html',
  styleUrl: './view-purchase-invoices.component.css'
})

export class ViewPurchaseInvoicesComponent {
  ...
  getPurchaseInvoices(): void {
    this._apiCalling.getData("invoice", "purchase/" + this._authService.getUserId(), true)
  ...
  navigate(type: number, invoice: any): void {
    if (type === 1) {
      this._router.navigate([`${'/invoice/add-purchase-invoice'}`]);
    } else {
      ...
      this._router.navigateByUrl(`${`/invoice/edit-purchase-invoice?invoiceId=${invoice.invoiceId}`}`);
    }
  }
  ...
  downloadExcel(): void {
    if (this.invoicesList.length > 0) {
      ...
      for (var k = 0; k < this.invoicesList.length; k++) {
        Data.push({
          "SrNumber": 'IVP-' + this.padNumberWithZeros(SrNumber),
      ...
      XLSX.utils.book_append_sheet(wb, ws, "PurchaseInvoice");
      var wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
      FileSaver.saveAs(new Blob([wbout], { type: "application/octet-stream" }), 'PurchaseInvoice' + ".xlsx");
    }
  }

...
@Component({
  selector: 'app-view-bank-sales-receipt',
  standalone: true,
  imports: [CommonModule, FilterPipe, HighlightPipe, FormsModule, TranslateModule],
  templateUrl: './view-bank-sales-receipt.component.html',
  styleUrl: './view-bank-sales-receipt.component.css'
})
export class ViewBankSalesReceiptComponent {
  ...
  getBankReceipts(): void {
    this._apiCalling.getData("bankReceipt", `getSaleReceipt/${this._authService.getUserId()}`, true)
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe({
        next: (response) => {
          if (response?.success) {
            this.allBankReceipts = response.data;
            console.log(this.allBankReceipts);
          } else {
  ...
  navigate(type: number, bank: any): void {
    if (type === 1) {
      this._router.navigate([`${'/bank/add-sale-receipt'}`]);
    } else {
      if (isPlatformBrowser(this.platformId)) {
        localStorage.removeItem('bank');
        localStorage.setItem('bank', JSON.stringify(bank));
      }
      this._router.navigateByUrl(`${`/bank/edit-sale-receipt?receiptId=${bank.receiptId}`}`);
    }
  }

  deleteConfirmation(loc: any): void {
    this.selectedObj = {};
    this.selectedObj.receiptId = loc.receiptId;
  ...
  downloadExcel(): void {
    if (this.allBankReceipts.length > 0) {
      ...
      for (var k = 0; k < this.allBankReceipts.length; k++) {
        Data.push({
          "SrNumber": 'BRS-' + this.padNumberWithZeros(SrNumber),
      ...
      XLSX.utils.book_append_sheet(wb, ws, "PurchaseSaleReceipt");
      var wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
      FileSaver.saveAs(new Blob([wbout], { type: "application/octet-stream" }), 'PurchaseSaleReceipt' + ".xlsx");
    }
  }

...
@Component({
  selector: 'app-add-edit-bank-sales-receipt',
  standalone: true,
  imports: [NgxFileDropModule, CommonModule, FormsModule, ReactiveFormsModule, AddEditCustomerComponent, NgbDatepickerModule, AddEditBankComponent, NgSelectModule, TranslateModule],
  templateUrl: './add-edit-bank-sales-receipt.component.html',
  styleUrl: './add-edit-bank-sales-receipt.component.css'
})
export class AddEditBankSalesReceiptComponent {
      ...
      .pipe(takeUntil(this.ngUnsubscribe)).subscribe({
        next: (response) => {
          if (response?.success) {
            ...
            if (customerId !== 0) {
              this.bankReceiptForm.controls['selectedId'].setValue(customerId);
  ...
  back(): void {
    this._router.navigate([`${'/bank/bank-sale-receipts'}`]);
  }

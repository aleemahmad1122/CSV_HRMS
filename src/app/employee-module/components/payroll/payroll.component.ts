...
export class PayrollComponent{
  ...
  private loadPermissions(): void {
    this.activatedRoute.data.subscribe(data => {
      ...
      if (Array.isArray(permissionsData)) {
        ...
        this.isDelete = this.permissions.some(p => p.permission === "Delete_Employee_Payroll" && p.isAssign);
      } else {
    ...
    private getActiveStatusData(searchTerm = '', isActive: number | string = 0): void {
      ...
      this.apiService.getData('EmployeePayroll', 'getEmployeePayroll', true, { searchQuery: searchTerm, activeStatus: isActive,employeeId:this.id })
  ...
  private getData(searchTerm = ''): void {
    this.apiService.getData('EmployeePayroll', 'getEmployeePayroll', true, { searchQuery: searchTerm,employeeId:this.id  })
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res: IEmployeeWorkHistoryRes) => {this.handleResponse(res);    this.generatePages()},
        error: () => (this.dataList = []),
  ...
  private getPaginatedData(): void {
    ...
    this.apiService.getData('EmployeePayroll', 'getEmployeePayroll', true, params)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
  ...
  onDelete(id: string): void {
    this.apiService.deleteData('EmployeePayroll', `deleteEmployeeWorkHistory/${id}`, {},true,this.id)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res) => {

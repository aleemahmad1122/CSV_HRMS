...
import { IAttachmentType,IAttachmentTypeRes } from '../../../types/index';
...
export class ListComponent   {
  ...
  private loadPermissions(): void {
    this.activatedRoute.data.subscribe(data => {
      ...
      if (Array.isArray(permissionsData)) {
        ...
        this.isEdit = this.permissions.some(p => p.permission === "Edit_Attachment_Type" && p.isAssign);
        this.isCreate = this.permissions.some(p => p.permission === "Create_Attachment_Type" && p.isAssign);
        this.isDelete = this.permissions.some(p => p.permission === "Delete_Attachment_Type" && p.isAssign);
      } else {
    ...
    private getActiveStatusData(searchTerm = '', isActive: number | string = 0): void {
      ...
      this.apiService.getData('AttachmentType', 'getAttachmentType', true, { searchQuery: searchTerm, activeStatus: isActive })
        ...
        .subscribe({
          next: (res: IAttachmentTypeRes) => {this.handleResponse(res); this.generatePages()},
          error: () => (this.dataList = []),
        });
    }
  ...
  private handleResponse(response: IAttachmentTypeRes): void {
    if (response?.success) {
      const { attachmentTypes, pagination } = response.data;
      Object.assign(this, {
        dataList: attachmentTypes,
  ...
  private getPaginatedData(): void {
    ...
    this.apiService.getData('AttachmentType', 'getAttachmentType', true, params)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
  ...
  onDelete(id: string): void {
    ...
    this.apiService.deleteData('AttachmentType', `deleteAttachmentType/${id}`, id, true)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res) => {

...
export class QualificationsListComponent {
  ...
  private loadPermissions(): void {
    this.activatedRoute.data.subscribe(data => {
      ...
      if (Array.isArray(permissionsData)) {
        ...
        this.isEdit = this.permissions.some(p => p.permission === "Edit_Qualification" && p.isAssign);
        this.isCreate = this.permissions.some(p => p.permission === "Create_Qualification" && p.isAssign);
        this.isDelete = this.permissions.some(p => p.permission === "Delete_Qualification" && p.isAssign);
      } else {
  ...
  private getData(searchTerm = ''): void {
    this.apiService.getData('Qualification', 'getQualifications', true, { searchQuery: searchTerm })
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res: IQualificationRes) => {this.handleResponse(res);    this.generatePages()},
        error: () => (this.dataList = []),
      });
  }

...
export class ListComponent {
  ...
  sortField: string = 'designationTitle';
  ...
  private loadPermissions(): void {
    this.activatedRoute.data.subscribe(data => {
      ...
      if (Array.isArray(permissionsData)) {
        ...
        this.isEdit = this.permissions.some(p => p.permission === "Edit_Designation" && p.isAssign);
        this.isCreate = this.permissions.some(p => p.permission === "Create_Designation" && p.isAssign);
        this.isDelete = this.permissions.some(p => p.permission === "Delete_Designation" && p.isAssign);
      } else {
    ...
    private getActiveStatusData(searchTerm = '', isActive: number | string = 0): void {
      ...
      this.apiService.getData('Designation', 'getDesignations', true, { searchQuery: searchTerm, activeStatus: isActive })
        ...
        .subscribe({
          next: (res: IDesignationRes) => {this.handleResponse(res);    this.generatePages()},
          error: () => (this.dataList = []),
        });
    }
  ...
  onDelete(id: string): void {
    ...
    this.apiService.deleteData('Designation', `deleteDesignation/${id}`, id, true)
      ...
      .subscribe({
        next: (res) => {
          if (res?.success){ this.dataList = this.dataList.filter((d) => d.designationId !== id)}else{

  ...
  private loadPermissions(): void {
    this.activatedRoute.data.subscribe(data => {
      ...
      if (Array.isArray(permissionsData)) {
        ...
        this.isEdit = this.permissions.some(p => p.permission === "Edit_Department" && p.isAssign);
        this.isCreate = this.permissions.some(p => p.permission === "Create_Department" && p.isAssign);
        this.isDelete = this.permissions.some(p => p.permission === "Delete_Department" && p.isAssign);
      } else {
    ...
    private getActiveStatusData(searchTerm = '', isActive: number | string = 0): void {
      ...
      this.apiService.getData('Department', 'getDepartments', true, { searchQuery: searchTerm, activeStatus: isActive })
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe({
          next: (res: IDepartmentRes) => {this.handleResponse(res);     this.generatePages()},
          error: () => (this.dataList = []),
        });
    }

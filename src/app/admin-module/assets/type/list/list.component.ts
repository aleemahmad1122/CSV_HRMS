...
import { IAssetTypeRes,IAssetType } from '../../../../types/index';
...
export class ListComponent  {
  ...
  private loadPermissions(): void {
    this.activatedRoute.data.subscribe(data => {
      ...
      if (Array.isArray(permissionsData)) {
        ...
        this.isEdit = this.permissions.some(p => p.permission === "Edit_Asset_Type" && p.isAssign);
        this.isCreate = this.permissions.some(p => p.permission === "Create_Asset_Type" && p.isAssign);
        this.isDelete = this.permissions.some(p => p.permission === "Delete_Asset_Type" && p.isAssign);
      } else {
  ...
  private getActiveStatusData(searchTerm = '', isActive: number | string = 0): void {
    ...
    this.apiService.getData('AssetType', 'getAssetType', true, { searchQuery: searchTerm, activeStatus: isActive })
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res: IAssetTypeRes) => {this.handleResponse(res);this.generatePages()},
        error: () => (this.dataList = []),
      });
  }
  ...
  private getData(searchTerm = ''): void {
    this.apiService.getData('AssetType', 'getAssetType', true, { searchQuery: searchTerm })
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res: IAssetTypeRes) => {this.handleResponse(res);this.generatePages()},
        error: () => (this.dataList = []),
  ...
  private handleResponse(response: IAssetTypeRes): void {
    if (response?.success) {
      const { assetTypes, pagination } = response.data;
      Object.assign(this, {
        dataList: assetTypes,
  ...
  private getPaginatedData(): void {
    ...
    this.apiService.getData('AssetType', 'getAssetType', true, params)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
  ...
  onDelete(id: string): void {
    this.apiService.deleteData('AssetType', `deleteAssetType/${id}`, id, true)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res) => {
          if (res?.success){ this.dataList = this.dataList.filter((d) => d.assetTypeId !== id)}else{
